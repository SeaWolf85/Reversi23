...
socket.on('join_room_response',(payload)=>{
    ...
    //if we are being notified of ourselves then ignor the message.
    ...
    */
    ...
    let newHTML = '<p class= \'join_room_response\'>'+payload.username+ ' joined the the chatroom. (There are '+payload.count+ ' users in this room)</p>';
    let newNode= $(newHTML);
    newNode.hide();
...
socket.on('player_disconnected',(payload)=>{
    ...
    let domElements =$('.socket_'+payload.socket_id);
    console.log(domElements.length + '   This is the length of DomElements');
    if(domElements.length !== 0){
        domElements.fadeOut(500);
...
socket.on('game_update',(payload)=>{
    ...
    if(my_color === 'white'){
        $('#my_color').html('<h3 id = "my_color" style="font-weight: bold; color: white;"> I am white </h3>');
        console.log("!! My color is "+my_color);
    }
    else if(my_color === 'black') {
        $('#my_color').html('<h3 id = "my_color" style="font-weight: bold; color: black;"> I am black </h3>');
        console.log('!! My color is '+my_color);
    }
    else{
    ...
    if(payload.game.whose_turn === 'white'){
        $('#my_color').append('<h4 style="font-weight:bold;"> It is white\'s turn </h4>');
        console.log('! It\'s my turn'+payload.game.whose_turn );
    }
    else if(payload.game.whose_turn === 'black') {
        $('#my_color').append('<h4 style="font-weight:bold;"> It is black\'s turn </h4>');
        console.log('! It\'s my turn'+payload.game.whose_turn );
    }
    else{
   ...
   let blacksum =0; 
    ...
    console.log("old board " + old_board);
    for (let row =0; row < 8; row ++){
        for (let column = 0; column < 8; column ++){
            ...
            //Check to see if the serve changed any space on the board
            if(old_board[row][column] !== board[row][column]){
                ...
                if((old_board[row][column] ==='?') && (board[row][column] === ' ')) {
                    ...
                    graphic = 'empty_to_white.gif';
                    console.log("XXXXXX Yo!")
                    altTag = 'white token';
                }
                else if((old_board[row][column] ===' ') && (board[row][column] === 'b')) {
                    console.log("XXXXX Black");
                    graphic = 'empty_to_black.gif';
                    altTag = 'black token';
                }
                ...
                if(payload.game.whose_turn === my_color){
                    ...
                    $('#'+row+'_'+column).click(((r, c) => {
                        return(()=>{
                            ...
                            console.log('*** Client log message, sending\'play_token\' commands: ' +JSON.stringify(payload));
