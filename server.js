...
io.on('connection', (socket) => {
    ...
    */
    ...
    'socket_id: the socket of the user that just joinded the room
    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)) {
            ...
            serverLog('join_room_command_failed', JSON.stringify(response));
            return;
        }
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)) {
            ...
            serverLog('invite_command_failed', JSON.stringify(response));
            return;
        }
        ...
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
                result : 'fail',
                message : 'the user that was invited does not have a name registerd'
            }  
            socket.emit('invite_response', response);
            serverLog('invite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('uninvite', (payload) => {
        ...
        if ((typeof payload == 'undefined') || (payload === null)) {
            ...
            serverLog('uninvite_command_failed', JSON.stringify(response));
            return;
        }
        let requested_user = payload.requested_user;
        let room = players[socket.id].room;
        ...
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
                result : 'fail',
                message : 'the user that was uninvited does not have a name registerd'
            }  
            socket.emit('uninvited', response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
                result : 'fail',
                message : 'the user that was engaged to play does not have a name registerd'
            }  
            socket.emit('game_start_response', response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
        ...
        //Make sure the player to engage  is still present
        io.in(room).allSockets().then((sockets) => {
    ...
    socket.on('disconnect', () => {
        ...
        if((typeof players[socket.id] != 'undefinded') && (players[socket.id] != null)){
            let payload = {
    ...
    //Check that the data coming from the client is good.
    /*send_chat_message to command handler 
      expected payload: 
      {
        'room': the room to which the message should be sent,
        'username': the name of the sendeer,
    ...
    */
    ...
    socket.on('send_chat_message', (payload) => {
        ...
        //Tell everyone inthe room wath the message is
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'play token command did not come from a registered usernem';
            socket.emit('play_token_response', response);
            serverLog('play_token_command_failed', JSON.stringify(response));
            return;
        }
        ...
        // Make sure the current attempt is by the current color
        ...
        if (
            ...
            socket.emit('play_token_response', response);
            socket.emit('whose turn ==='+game.whose_turn);
            serverLog('play_token_command_failed', JSON.stringify(response));
            return;
        }
...
function check_line_match(color,dr,dc,r,c,board){
    ...
    //check to make sure we aren't going to walk off of the board
...
//Return true if r+ dr supports playing playing at r and c + dc supports playing that move
